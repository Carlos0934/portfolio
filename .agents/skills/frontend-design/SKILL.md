...
description: Create distinctive, production-grade frontend interfaces aligned with the portfolio's Operator Core dark-first design system. Use this skill when the user asks to build or refine web pages, sections, components, layouts, or UI styling in Astro/Tailwind. Generates polished interfaces with strong hierarchy, restrained surfaces, and a serious systems-oriented visual language.
license: Complete terms in LICENSE.txt
---

This skill guides creation of production-grade frontend interfaces for this portfolio. The complete design system — colors, typography, components, layout, elevation, and responsive rules — lives in `DESIGN.md`. **Read it first.** This skill only covers workflow, project conventions, and guidance not present in that document.
...
- **read `DESIGN.md` before any major UI decision** — it is the source of truth for the design system
- **read `src/styles/global.css` when changing tokens, theme, or global visual behavior**
- keep the portfolio inside the Operator Core aesthetic and structural rules defined in `DESIGN.md`
...
- never redesign a component without checking `DESIGN.md` and `src/styles/global.css` first
- never introduce a visual direction that conflicts with the Operator Core system
...
- **[Operator Core design system reference](DESIGN.md)** — the primary source of truth for all visual decisions
- **`src/styles/global.css`** — the active theme tokens and animation utilities
...
1. **Always read `DESIGN.md` first**
   - Use it for principles, visual tone, component rules, review criteria, color values, typography specs, spacing, and layout guidance.
   - Section 9 ("Agent Prompt Guide") contains quick references and example component prompts.
...
Before coding, understand the context and choose a direction that stays inside the Operator Core system rather than inventing a random aesthetic:
...
- Cohesive with the Operator Core design system
...
- **Reference `DESIGN.md` sections 4 and 5 for component styling and layout specifics** — do not invent spacing, radius, or color rules that contradict it.
...
- maintain consistency with `DESIGN.md` and the project theme source
...
- Do not redesign components in isolation without checking `DESIGN.md` first.
- Do not introduce visual patterns that fight the Operator Core system.
...
6. Was the relevant source of truth read first (`DESIGN.md`, `src/styles/global.css` when applicable)?

## Resources

- **Design system**: `DESIGN.md` (Operator Core dark-first design system)
